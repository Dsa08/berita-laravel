                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" value="{{ old('title', $post->title) }}" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" required>
                        </div>

                        <div class="mb-4">
                            <label class="block font-medium text-sm text-gray-700">Isi Berita</label>
                            <textarea name="content" rows="8" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm mt-1 block w-full" required>{{ old('content', $post->content) }}</textarea>
                        </div>

                        <div class="mb-4">
                            <label class="block font-medium text-sm text-gray-700 mb-2">Thumbnail Sekarang:</label>
                            @if ($post->thumbnail)
                                <img src="{{ asset('storage/' . $post->thumbnail) }}" class="rounded shadow-md mb-2 w-48 h-auto border">
                            @else
                                <p class="text-sm text-gray-500 italic bg-gray-50 p-2 rounded">Belum ada thumbnail</p>
                            @endif
                        </div>

                        <div class="mb-4">
                            <label class="block font-medium text-sm text-gray-700">Ganti Thumbnail (Opsional)</label>
